#include <stdint.h>
#include "./sys_dram.h"
#include "../f1c100s/arm926/inc/io.h"
...
static int dram_para_setup(struct dram_para_t* para) {
    ...
    val = (para->ddr8_remap) | (0x1 << 1) | ((para->bank_size >> 2) << 3) | ((para->cs_num >> 1) << 4) | ((para->row_width - 1) << 5) | ((para->col_width - 1) << 9) | ((para->sdr_ddr ? (para->bwidth >> 4) : (para->bwidth >> 5)) << 13) | (para->access_mode << 15) | (para->sdr_ddr << 16);
...
static uint32_t dram_scan_readpipe(struct dram_para_t* para) {
    ...
    if (para->sdr_ddr == DRAM_TYPE_DDR) {
        for (i = 0; i < 8; i++) {
            ...
            dram_delay_scan();
            readpipe[i] = 0;
            if ((((read32(DRAMC_BASE + DRAM_DDLYR) >> 4) & 0x3) == 0x0) && (((read32(DRAMC_BASE + DRAM_DDLYR) >> 4) & 0x1) == 0x0)) {
